...
@php
    $user = $user ?? Auth::user();
@endphp
<div class="px-6 py-8" style="background: linear-gradient(135deg, #0b1f3a 0%, #14335a 100%); font-family: 'Poppins', sans-serif;">
    <div class="flex items-center space-x-6">
        <div class="relative group">
            @if($user->avatar_url)
                <img class="h-24 w-24 rounded-full object-cover border-4 border-white shadow-lg transition-transform duration-300 group-hover:scale-105"
                    ...
                    onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&color=ffffff&background=0b1f3a&size=128&font-size=0.4'">
            @else
                <img class="h-24 w-24 rounded-full object-cover border-4 border-white shadow-lg transition-transform duration-300 group-hover:scale-105"
                    src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&color=ffffff&background=0b1f3a&size=128&font-size=0.4"
                    alt="Profile {{ $user->name }}">
            @endif
        ...
        <div class="text-white flex-1">
            ...
            <p class="font-medium mt-1" style="color: #e8d48b; font-family: 'Poppins', sans-serif; font-weight: 500;">
                {{ $user->email }}
            </p>
            <div class="mt-3 flex items-center space-x-4">
                <span class="px-3 py-1 rounded-full text-sm font-medium transition-all duration-200" style="background-color: rgba(201, 162, 39, 0.35); color: white; font-family: 'Poppins', sans-serif;">
        ...
        <div class="hidden md:flex flex-col items-end space-y-2">
            <div class="text-right" style="color: #e8d48b;">
